...
#define __ckd_declare_add(S, T) \
    ...
    { \
        ...
        *(T*)__res = __z; \
        ...
        int __case = (__ckd_is_signed((T)0) << 2) | (__a_signed << 1) | __b_signed; \
        if (__case == 0) { /* u = u + u */ \
            return __truncated | (__z < __x); \
        } else if (__case == 1) { /* u = u + s */ \
